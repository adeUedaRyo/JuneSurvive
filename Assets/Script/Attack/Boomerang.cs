...
public class Boomerang : MonoBehaviour
{
    ...
    [SerializeField] float _speed = 5;//回転速度
    ...
    void Update()
    {
        ...
        if (transform.position.x >= player.transform.position.x + 6 || transform.position.x <= player.transform.position.x - 6 || transform.position.y >= player.transform.position.y + 4 || transform.position.y <= player.transform.position.y - 4)
        {
